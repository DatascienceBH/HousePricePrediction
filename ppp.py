...
y_pred = y_pred_xgb * (1 / 2) + y_pred_lasso * (1 / 2)

y_pred = np.exp(y_pred)
y_pred_lasso = np.exp(y_pred_lasso)

pred_df = pd.DataFrame(y_pred, index=test_df["Id"], columns=["SalePrice"])
y_pred_lasso = pd.DataFrame(y_pred_lasso, index=test_df["Id"], columns=["SalePrice"])

# pred_df.to_csv('G:/southampton/data science/semester2/Advanced machine learning/House prices predictionoutput.csv', header=True, index_label='Id')
# y_pred_lasso.to_csv('G:/southampton/data science/semester2/Advanced machine learning/lassooutput.csv', header=True, index_label='Id')
...
trainX=train_df_munged
trainY=label_df
testX=test_df_munged
...
xgtest = xgb.DMatrix(testX.values)
